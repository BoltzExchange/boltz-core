...
docker run -v `pwd`/docker:/config -d --name $bitcoin_container -p 18443:18443 boltz/bitcoin-core:29.0 $bitcoin_config > /dev/null
...
echo "Creating container"
docker run -v `pwd`/docker:/config -d --name $elements_container -p 18884:18884 boltz/elements:23.2.7 $elements_config > /dev/null
