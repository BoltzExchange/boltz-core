...
contract ERC20SwapFuzzTest is Test {
    ...
    function testLock(uint256 amount, bytes32 preimageHash, address claimAddress, uint256 timelock) external {
        ...
        assertTrue(swap.swaps(swap.hashValues(preimageHash, amount, address(token), claimAddress, address(this), timelock)));
    }

    function testLockPrepayMinerFee(
        uint256 amount, uint256 prepayAmount, bytes32 preimageHash, address payable claimAddress, uint256 timelock
    ) external {
    ...
    function testLockClaim(uint256 amount, bytes32 preimage, address claimAddress, uint256 timelock) external {
        ...
        assertFalse(swap.swaps(swap.hashValues(preimageHash, amount, address(token), claimAddress, address(this), timelock)));
    }

    function testLockClaimInvalidPreimage(
        uint256 amount, bytes32 preimage, bytes32 invalidPreimage, address payable claimAddress, uint256 timelock
    ) external {
        vm.assume(amount < token.balanceOf(address(this)));
        vm.assume(amount > 0);
    ...
    function testLockRefund(uint256 amount, bytes32 preimageHash, address payable claimAddress, uint256 timelock) external {
        vm.roll(6_573_352);
    ...
    function testLockRefundTimelockNotPassed(
        uint256 amount, bytes32 preimageHash, address payable claimAddress, uint256 timelock
    ) external {
        vm.roll(6_573_352);
    ...
    function testLockRefundCooperativeInvalidSignature(
        uint256 amount, bytes32 preimageHash, address payable claimAddress, uint256 timelock, uint8 v, bytes32 r, bytes32 s
    ) external {
        vm.assume(amount < token.balanceOf(address(this)));
        vm.assume(amount > 0);
